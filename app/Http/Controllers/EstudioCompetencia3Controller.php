...
class EstudioCompetencia3Controller extends Controller
{
    ...
    public function rankingEmpresa($unidad, $periodo, $empresa, $uso)
    {
        ...
        $data = DB::connection('mysql2')->table('datos')
            ->join('anunciantes_productos', 'datos.producto2', '=', 'anunciantes_productos.id')
            ->join('lugares', 'lugares.id', '=', 'datos.lugares_id')
            ->join('propietarios', 'propietarios.id', '=', 'lugares.propietarios_id')
            ->join('clientes', 'clientes.id', '=', 'datos.clientes_id')
            ->select(
            ...
            ->groupBy('lugares.propietarios_id')
            ->get();
    ...
    public function validacionReporte($unidad, $periodo, $empresa, $uso){
        ...
        $lugares = DB::connection('mysql2')->table('lugares')
            ->select(
                'propietarios_id',
                'id',
                'direccion'
        ...
        {
            if(isset($resultado[$value->propietarios_id])){
                $resultado[$value->propietarios_id]['inventario']++;
                if(isset($data[$value->id])){
                    if(!isset($resultado[$value->propietarios_id]['reporte'])){
                        $resultado[$value->propietarios_id]['reporte'] = 1;
                    }else{
                        $resultado[$value->propietarios_id]['reporte']++;
                    }
                }else{
                    if(!isset($resultado[$value->propietarios_id]['ubicaciones'])){
                        $resultado[$value->propietarios_id]['ubicaciones'] = [];
                    }
                    array_push($resultado[$value->propietarios_id]['ubicaciones'], $value->direccion.' ['.$value->id.']');
                }
            }else{
                $resultado[$value->propietarios_id]['inventario']=1;
                if(isset($data[$value->id])){
                    if(!isset($resultado[$value->propietarios_id]['reporte'])){
                        $resultado[$value->propietarios_id]['reporte'] = 1;
                    }else{
