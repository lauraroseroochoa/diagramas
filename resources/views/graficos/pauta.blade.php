        ...
        <table id="pauta" class="datatable1 table table-striped tableFont w-100 "  style="font-size: 12px;">
        	<thead>
        		<tr>
        			...
        			<th class="border">Comercializador</th>
        	...
        	<tbody>
        		@foreach($datos as $key => $value)
        			<tr>
	        			...
	        			<td class="border">{{ isset($empresas[$value->comercializadorId])?$empresas[$value->comercializadorId]->descripcion:'' }}</td>
	        			...
	        			<td class="border"><i class="fa-solid fa-image" onclick="showPhoto('{{$value->foto2 }}','{{ $empresas[$value->comercializadorId]->descripcion }} - {{ $value->direccion }} - {{ $value->created_at }}');"></i></td>
	        		</tr>
        		@endforeach
        	</tbody>
...
<script type="text/javascript">
    ...
    @foreach($datos as $value)
            ...
            @endphp
                ...
                L.marker([{{ $lat }}, {{ $lng }}])
                    .addTo(map)
                    .bindPopup("Empresa: {{ $empresas[$value->comercializadorId]->descripcion }} ");
