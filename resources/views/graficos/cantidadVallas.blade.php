<div class="row">
	<div class="col-7" style="height: 65vh; max-height: 65vh;  ">
        <div class="mt-3 " style="margin-bottom:-32px;"><h5 class="text-dos" >Cantidad {{ $descr }}</h5></div>
		<table id="cantidad" class="datatable1 table table-striped tableFont w-100 ">
			...
			<tbody>
				...
				<tr>
					...
					<td class="border"><a href="#" onclick="graficarMarkers({{$key}},'datosTradicional');">{{ $value['tradicional'] }}</a></td>
					<td class="border"><a href="#" onclick="graficarMarkers({{$key}},'datosLed');">{{ $value['led'] }}</a></td>
					<td class="border"><a href="#" onclick="graficarMarkers({{$key}},'datos');">{{ $value['total'] }}</a></td>
				</tr>
				@endforeach
			</tbody>
			<tfoot>
    ...
    @foreach($datos as $data)
    	@foreach($data['datos'] as $key => $value)
		    ...
		    @endphp
		        ...
		        L.marker([{{ $lat }}, {{ $lng }}])
		            .addTo(map)
		            .bindPopup("Empresa: {{ $value->propietario }} <br>  Direccion: <br>   Sentido: {{ $sentidos[$value->sentidos_id]}}<br>");
		@endforeach
	@endforeach
